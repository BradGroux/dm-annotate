...
Annotation sessions can be saved to and loaded from local `.dmannotate-session` JSON files. The app preserves annotation geometry, display IDs and logical-point display geometry, colors, stroke widths, text styles, visibility, lock state, and whiteboard state. Loading and live screen-parameter changes use the same Core retargeting module when a display is missing. Existing-display annotations remain unchanged. Missing-display annotations map the normalized center of their exact painted bounds from the saved/source usable bounds into the main destination's usable bounds, preserving painted size in logical points across backing-scale changes. The result is clamped to fit when possible; geometry larger than a destination axis is centered on that axis so it remains intersecting and usable. If source geometry is unavailable, painted bounds are centered in the destination. If the requested fallback is unavailable, the lowest available display ID is deterministic fallback; with no displays, annotations remain unchanged until a usable destination appears. A reconnected source does not pull an annotation back after it has been retargeted. System retargeting clears edit history so undo cannot restore disconnected display IDs. The Core session boundary applies the same annotation, point, text, style, geometry, color, and encoded-byte validation before export and after import. Store exports validate before an atomic file replacement, and live freehand strokes are progressively simplified to remain within the session point limit.

High-frequency annotation work uses explicit bounds. `AnnotationItem` owns the effective highlighter width, painted stroke radius, smoothed cubic stroke segments, and arrowhead geometry shared by AppKit rendering, painted bounds, selection, hit testing, eraser culling, and invalidation. Stroke interaction combines the painted radius, antialias tolerance, and caller-provided selection or eraser radius. Cubic hit testing uses adaptive subdivision, while chunk bounds conservatively include each segment's control hull. The store caches each annotation's display-space bounds plus 64-segment spatial chunks and updates that index across edits, load, undo, and redo. Eraser input rejects both annotation bounds and chunk bounds before detailed segment hit testing. Overlay redraws query the bounds index for the dirty region; text remains conservatively uncullable and text mutations request a safe full redraw until AppKit layout metrics are shared. Inline text entry uses one deterministic presentation model to select a solid black or white editor surface with at least 4.5:1 contrast against the calibrated preview color's display-resolved sRGB components. The same preview drives text, insertion point, border, and selection background; selected text inverts to the editor surface. The editor snapshots its RGBA color when entry begins, so palette changes cannot alter either its preview or committed color. Increase Contrast doubles the static border width, and active editors refresh when macOS posts an accessibility display-options change. The editor preview is opaque for legibility, while commit stores the original session RGBA color unchanged. Capture hides the complete overlay window before acquiring screen pixels, then redraws only committed annotations from the store, so editor chrome is not part of app-generated captures. Persisted freehand paths are cached by annotation ID, and targeted store invalidations evict only changed paths and redraw only their old/new regions. Freehand input uses the session-compatible 20,000-point cap with geometry-aware reduction. Laser input retains at most 240 spatially distinct samples over 1.5 seconds and owns one stable cleanup timer per overlay.
